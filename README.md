...
CRL02 最终闭合光滑网格同时用于显示、长方体、最大贴合和 STL 判定；设备显示空区 `X=-2335…1895、Y=-1469.927…1469.926、Z≥0` 作为页面打印空间排除项。该空区来自控制模型的显示规则，不应解读为完整物理碰撞或安全净距模型。CRL02 十个设备分件采用高精度 `KHR_mesh_quantization`，不减面，并用压缩后的同一字节重建 HTTP 与 `file://` 资源。
